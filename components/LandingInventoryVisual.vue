<template>
  <div ref="root" class="surface rounded-3xl p-5 sm:p-7">
    <div class="flex items-center justify-between gap-3 pb-5 border-b border-black/[0.06]">
      ...
      <span ref="syncChip" class="chip chip-accent shrink-0 flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-[1fr_auto_1fr] items-stretch gap-2 sm:gap-3 py-6">
      ...
      <div class="relative self-center w-10 sm:w-16 h-px bg-black/[0.12]" aria-hidden="true">
        <div ref="pulse" class="absolute inset-y-0 left-0 w-full origin-left bg-pokemon-red" style="transform: scaleX(0)" />
        <div ref="pulseDot" class="absolute -top-[3px] left-0 w-[7px] h-[7px] rounded-full bg-pokemon-red opacity-0" />
      </div>
      ...
      >
        ...
        <p class="mt-1 text-lg font-bold text-ink tabular-price">RM 245</p>
        <div class="mt-3 rounded-lg border border-black/[0.08] bg-white text-ink-muted text-[11px] font-semibold text-center py-2">
          Delisted
        </div>
      </div>
    </div>

    <div class="flex items-center justify-between gap-3 pt-5 border-t border-black/[0.06]">
      <p class="text-xs text-ink-muted">One stock ledger</p>
      <p class="flex items-baseline gap-1.5">
        <span ref="stockNum" class="text-2xl font-bold text-ink tabular-price leading-none">12</span>
...
useReveal(root, ({ gsap, reduced, root: el }) => {
  ...
  tl.to(chargeBtn.value!, { scale: 0.95, duration: 0.12, ease: "power2.in" })
    ...
    .fromTo(posPanel.value!, { boxShadow: "0 0 0 0px rgba(227,53,13,0)" }, { boxShadow: "0 0 0 2px rgba(227,53,13,0.5)", duration: 0.3 }, "<")
    .to(pulse.value!, { scaleX: 1, duration: 0.5, ease: "power2.inOut" }, "-=0.1")
    .fromTo(pulseDot.value!, { opacity: 1, x: 0 }, { x: "100%", duration: 0.5, ease: "power2.inOut" }, "<")
    .to(pulseDot.value!, { opacity: 0, duration: 0.15 })
    .fromTo(onlinePanel.value!, { boxShadow: "0 0 0 0px rgba(227,53,13,0)" }, { boxShadow: "0 0 0 2px rgba(227,53,13,0.5)", duration: 0.3 }, "<")
    ...
    .fromTo(syncChip.value!, { scale: 1 }, { scale: 1.08, duration: 0.18, yoyo: true, repeat: 1 }, "<")
    .to([posPanel.value!, onlinePanel.value!], { boxShadow: "0 0 0 0px rgba(227,53,13,0)", duration: 0.4 })
