<template>
  <div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-2 text-ink dark:text-white">Checkout</h1>
    ...
    <div v-if="items.length === 0" class="text-center py-16">
      <p class="text-gray-500 dark:text-zinc-400 text-lg">Nothing to check out.</p>
      <p class="text-sm text-gray-400 dark:text-zinc-500 mt-1">
        Choose items in your cart, or tap Buy Now on a listing.
      </p>
    ...
    <template v-else>
      ...
      <div class="surface rounded-2xl border border-black/[0.06] dark:border-white/[0.08] p-5">
        <div class="space-y-1 text-sm mb-4">
          <div class="flex justify-between text-gray-600 dark:text-zinc-300">
            ...
            <span class="tabular-nums">RM {{ checkoutTotal.toFixed(2) }}</span>
          </div>
          <div class="flex justify-between text-gray-600 dark:text-zinc-300">
      ...
      <!-- Selection happens in the cart drawer, not here. -->
      <button
        @click="router.back()"
        class="text-sm text-gray-400 dark:text-zinc-500 hover:text-ink dark:hover:text-white mt-4 block mx-auto"
      >
        {{ source === "buyNow" ? "Back to listing" : "Back to cart" }}
      </button>
    </template>
  </div>
</template>
...
useHead({ title: "Checkout | TCGo Marketplace" });

const router = useRouter();
// Only what is being checked out: the cart drawer's selection, or a single
// Buy Now listing that never entered the cart. See composables/useCheckout.ts.
const {
  checkoutItems: items,
  checkoutTotal,
  source,
...
const grandTotal = computed(() => checkoutTotal.value + totalShipping.value);
...
const handlePlaceOrders = async () => {
  ...
  try {
    ...
    // Remove only the checked-out items from the cart; a Buy Now leaves it alone.
    completeCheckout();
